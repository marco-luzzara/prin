...
if [ ! -e "${JAVA_BIN}" ]; then
  echo "$JAVA_BIN found on the system. Please make sure java and jar commands are available."
  exit 1
fi

# Construct Atlas classpath using jars from ${HIVE_HOOK_IMPL_DIR} directory.
for i in "${BASEDIR}/${HIVE_HOOK_IMPL_DIR}/"*.jar; do
